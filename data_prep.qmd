---
title: "Data prep: Rubenstein et al. 2023"
format: 
  html:
...
# rshifts_df_raw$speriod %>% table() ### some bare numbers, mostly P# format
  ...
  #   6     7     8     9    P1   P10   P11   P12   P13   P14   P15   P16 
  #   1     1     1     1 27862     5     5     5     5     5     5     5 
  # P17   P18   P19    P2   P20   P21   P22   P23   P24    P3    P4    P5 
  #   5     5     5  3222     5     5     1     1     1   693   292   105 
  #  P6    P7    P8    P9 
  # 134   166    56    25
# x <- rshifts_df_raw %>%
...
```{r}
polygon_keys <- c('polygon_name', 'speriod', 'ecotype')
poly_attrs_climate_df <- rshifts_df %>%
...
poly_clim_df <- poly_attrs_climate_df %>%
  select(polygon_name, climate_code) %>%
  distinct()

poly_attrs_df <- poly_attrs_climate_df %>%
  select(-climate_code) %>%
  distinct()
```
...
Separate out species taxonomic info from the climate and shift info using `species` as key.  Note, common name `cname` is highly variable and not 1:1 match to `species` in either direction, so doing a little clean up then combining all cnames into a single row.  Each species, in each polygon, is associated with one value for `casc` and one value for `data_source_title`, but multiple values for climate-related observations including `dim`, `obsvt`, etc.

```{r}
rshifts_df <- rshifts_df %>%
  ### some weird characters in there:
  mutate(sciname = str_remove_all(sciname, '�'))
  
sciname_comname_fix <- rshifts_df %>% 
  select(sciname, cname) %>% 
  distinct() %>%
  filter(!is.na(cname) & cname != 'n/a') %>%
  mutate(cname = str_to_sentence(cname) %>%
           stringi::stri_trans_general('Latin-ASCII'),  ### drop diacritics
         cname = str_remove_all(cname, '�'),
         cname = str_replace_all(cname, '-', ' ')) %>% 
  distinct() %>%
  group_by(sciname) %>%
  mutate(cname = paste(cname, collapse = ';')) %>%
  ungroup() %>%
  distinct()

x <- sciname_comname_fix %>%
  group_by(sciname) %>%
  filter(n_distinct(cname) > 1)
```


```{r isolate species and taxonomic info from range shifts}
tax_vars <- c('cname', 'tax', 'kingdom', 'phylum', 'class', 'order', 'family', 'genus', 'species', 'k2', 't2')
tax_keys <- c('sciname')

distinct_spp <- rshifts_df %>%
  select(-cname) %>%
  left_join(sciname_comname_fix, by = 'sciname') %>%
...
dbWriteTable(mydb, 'polygon_attrs', poly_attrs_df)
dbWriteTable(mydb, 'polygon_climate', poly_clim_df)
...
Check join columns in R:

```{r}
join_test <- range_shifts_clean %>%
  left_join(poly_attrs_df, by = c('polygon_name', 'speriod', 'ecotype')) %>%
  left_join(poly_clim_df, by = 'polygon_name', relationship = 'many-to-many') %>%
  left_join(spp_poly_df, by = c('polygon_name', 'speriod', 'ecotype', 'sciname'))
```
...
check <- anti_join(join_test, df_from_sql, 
                   by = c('polygon_name', 'paperid', 'speriod', 'ecotype', 'sciname'))

if(nrow(check) > 0) stop('The R join test and SQL join test do not match up!')
```
